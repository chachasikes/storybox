    ...
    AFRAME.registerComponent("storybox-material", {
            ...
            case "brush_Bubbles":
              material = this.tiltbrushMaterial(node, {
                ...
                texture: new THREE.TextureLoader().load(
                  "./images/textures/tiltbrush/Bubbles/maintexture.png"
                )
              });
              return material;
              break;
            case "brush_Stars":
              return this.pointMaterial(node, "./images/textures/tiltbrush/Stars/maintexture.png");
              break;
            case "brush_Rainbow":
            case "brush_Fire":
      ...
      addParallax: function(
        ...
        if (node.material[mapType] !== null) {
          let position = this.getClampedIncrement(
            node,
            increment,
            node.material[mapType],
            clamp
          );
        ...
        if (node !== undefined) {
          ...
          switch (materialNameUnique) {
            ...
            case "NeonPulse":
              node.time++;
              if (
                node.material.alphaMap !== undefined &&
                node.material.alphaMap !== null
              ) {
                node.material.alphaMap.offset.x = this.getOffset(
                  node,
                  0.001,
                  node.material.alphaMap.offset.x
                );
              }
              break;
            case "brush_Stars":
      ...
      tiltbrushMaterial: function(node, options = {}, e) {
        if (options.glow === true) {
          ...
          if (
            (node.material.map !== undefined && node.material.map !== null) ||
            ...
            if (texture !== undefined) {
              ...
              if (options.emissive === true) {
                ...
                material.emissive = new THREE.Color(
                  node.material.color.r,
                  node.material.color.g,
                  node.material.color.b
                );
      ...
      neonPulseMaterial: function(node) {
        var texture = new THREE.TextureLoader().load(
          "./images/textures/tiltbrush/NeonPulse/stripes.png"
        );
        var material = new THREE.MeshStandardMaterial({
          color: node.material.color,
          transparent: true,
      ...
      smokeMaterial: function(node) {
        var texture = new THREE.TextureLoader().load(
          "./../../images/textures/tiltbrush/Smoke/maintexture.png"
        );
        var material = new THREE.MeshStandardMaterial({
          color: node.material.color,
          transparent: true,
      ...
      pointMaterial: function(node, texture=null) {
        let map = new THREE.TextureLoader().load(texture);
        // console.log(node.material.map);

        if (texture !== null || node.material.map !== null) {
        let material = new THREE.PointsMaterial({
          map: map !== undefined ? map : node.material.map,
        ...
        let particles = new THREE.Points(node.geometry, material);
      ...
      computeFaceCentroids: function(geometry) {
        let f;
        let fl;
        let face;
      ...
      applyGlowMaterial(object, parent, isChild) {
        ...
        if (isChild === true) {
          object.glowMesh.rotation.set(
            this.el.object3D.rotation.x,
            this.el.object3D.rotation.y,
            this.el.object3D.rotation.z
          );
          object.glowMesh.scale.set(
            this.el.object3D.scale.x * this.data.glowScale,
            this.el.object3D.scale.y * this.data.glowScale,
            this.el.object3D.scale.z * this.data.glowScale
          );
          if (parent !== null) {
            //@todo position is wrong
            object.glowMesh.position.set(
              parent.position.x,
              parent.position.y,
              parent.position.z
            );
