AFRAME.registerComponent('glow', {
  // From https://github.com/etiennepinchon/aframe-glow
  ...
  init: function () {
    ...
    var run = function() {
      ...
      var mesh = that.el.getObject3D('mesh');
      var object = mesh.geometry;
      // console.log(object);
      if (object !== undefined) {
        let node = object.clone();
        applyMaterial(node, that);
      } else {
          mesh.traverse((node) => {
            if (node.geometry !== undefined) {
              let child = node.geometry.clone();
              applyMaterial(child, that);
            }
          });
      }
    }

    function applyMaterial(object, that) {
